        ...
        if (isset($_POST['save'])) { // if save button on the form is clicked
            ...
            if (!in_array($extension, ['zip', 'pdf', 'docx'])) {
                ...
                if (move_uploaded_file($file, $destination)) {
                    //$sql = "INSERT INTO tutor_data (fullname, size, downloads) VALUES ('$filename', $size, 0)";
                    $sql = "UPDATE `tutor_data` SET filename ='$filename' WHERE userid='".$_SESSION['user']['id']."'ORDER BY userid DESC";								
                         
                    //$sql = "UPDATE `tutor_data` SET fullname='$filename' WHERE userid='".$_SESSION['user']['id']."'ORDER BY userid DESC";
                    if (mysqli_query($conn, $sql)) {
                        echo "File uploaded successfully";
                    }
                } else {
                    echo "Failed to upload file.";
                // $yo3 = "UPDATE `tutor_data` SET fullname='$filename' WHERE userid='".$_SESSION['user']['id']."'ORDER BY userid DESC";
